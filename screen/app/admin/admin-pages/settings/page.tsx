...
import { User, Mail, Key, CheckCircle2 } from "lucide-react";
...
export default function AccountSettingsPage() {
  ...
  // ✅ Populate the form when the user arrives from the server
  useEffect(() => {
    if (user) {
      reset({
        name: user.name ?? "",
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 30 }}
            className="fixed bottom-6 right-6 bg-black text-white px-5 py-3 rounded-xl flex items-center gap-2 z-50"
          >
          ...
          <AnimatePresence>
            ...
            >
              <InputField
                ...
                {...registerPassword("current", { required: "Required" })}
