...
// ✅ Helper: check if a cookie exists in the browser
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // ✅ Skip interceptor entirely for auth routes
    ...
    // ================= GUEST GUARD =================
    // ✅ If this is a /auth/me call AND there's no refresh cookie,
    // the user is simply a guest. Fail quietly — DO NOT refresh, DO NOT redirect.
    const isMeRoute = originalRequest.url?.includes("/auth/me");
    if (isMeRoute && !hasRefreshCookie()) {
      return Promise.reject(error);
    }
    ...
    try {
      ...
      // 🚨 ONLY redirect to login if the user was actually on an admin route
...
export const apiClient = {
  ...
  post: async <T, B>(url: string, body: B, config?: any): Promise<T> => {
    try {
